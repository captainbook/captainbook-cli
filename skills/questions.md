...
Four other resources are in the same position — `guests`, `pricing-categories`, `pricing-tiers`, `categories`. Where `--since` *is* declared it still means a real lower bound on `updated_at` (`media list` excepted — see [media.md](media.md)), but plenty of endpoints simply never had one, `bookings list` among them. Check `--help` rather than assuming; [index.md](index.md#incremental-sync---since-is-not-universal) has the full picture.
...
- ⚠️ **No server-side dry-run on delete.** CLI rejects `--dry-run` at parse time. Gauge the impact with `answers list --question-id <id> --limit 1` and read the envelope's pagination rather than materialising every row: those rows are guest PII (passport numbers, DOBs, medical notes) and the endpoint needs `view_answers_of_booking` on top of `cli:read` — see [answers.md](answers.md). Note that `--question-id` there is **numeric**, not the prefixed `q_42` form this cookbook uses: pass the bare id.
